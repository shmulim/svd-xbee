...
tools.bArr2HexStr = function(a) {
    ...
    for(i in a) {
      if (a[i] != 0) leadingZero = false;
      if (!leadingZero) s += tools.dec2Hex(a[i]);
    }
    return s;
}
...
frames[C.FRAME_TYPE.AT_COMMAND_RESPONSE] = {
  parse: function(parser) {
    parser
      ...
      // TODO: Should we parse here, or seperately?
    if (parser.json.command == 'ND') {
      parser.json.commandData = {};
      parser.write = parser.json.commandData;
      parser
        .readAddr16('remote16')
        .readAddr64('remote64')
        .readString('id')
...
frames[C.FRAME_TYPE.ZIGBEE_IO_DATA_SAMPLE_RX] = {
  parse: function(parser) {
    parser
      ...
      .readAddr16('remote16')
      .readByte('receiveOptions')
      .collectPayload('rawIOSample');
    parser.json.ioSample = tools.parseIOSample(parser.json.rawIOSample);
  }
};

tools.parseIOSample = function(sample) {
  var res = {
    digitalSamples: {},
    analogSamples: {}
  }
  var numSamples = sample.splice(0,1)[0];
  var digitalChannelMask = sample.splice(0,2);
  var analogChannelMask = sample.splice(0,1)[0];
  var mskD = (digitalChannelMask[0] << 8) | digitalChannelMask[1];
  var mskA = analogChannelMask;

  if (mskD > 0) {
    var digitalSamples = sample.splice(0,2);
    var valD = (digitalSamples[0] << 8) | digitalSamples[1];
    for (var i in C.DIGITAL_CHANNELS) {
      if ((mskD & (1 << i)) >> i) {
        res.digitalSamples[C.DIGITAL_CHANNELS[i]] = (valD & (1 << i)) >> i;
      }
    }
  }
  ...
  if (mskA > 0) {
    ...
    var sampleNr = 0;
    for (var i in C.ANALOG_CHANNELS) {
      if ((mskA & (1 << i)) >> i) {
        var valA = (analogSamples[sampleNr*2] << 8) | analogSamples[sampleNr*2+1];
        // Convert to mV:
        res.analogSamples[C.ANALOG_CHANNELS[i]] = (valA * 1200) / 1023;
        sampleNr++;
      }
    }
  }
